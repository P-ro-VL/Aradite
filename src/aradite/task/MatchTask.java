...
import aradite.task.type.MechanicManagementTask;
...
public class MatchTask {
	...
	public void addTimerTask(Runnable task, boolean async, int time, int delay, Runnable callback) {
		...
		if (async) {
			bukkitTask = runnable.runTaskTimerAsynchronously(Aradite.getInstance(), delay, delay);
		} else {
			bukkitTask = runnable.runTaskTimer(Aradite.getInstance(), delay, delay);
		}
